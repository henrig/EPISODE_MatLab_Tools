function  write_EPISODE_traffic_Area_file(fname,area,Domain,TV,scale,ds,IDtag)
...
elseif size(TV,2)==2
    TVfrac=extractfield(Domain,'TVfrac');
    for t=1:h
        TEXT1=['H: ',num2str(t,'%6u')];
        fprintf(fid,'%10s%10s%3u%3u\n',TEXT1,'Area',ew,ns);
        for i=1:ns
            for j=1:ew
                fprintf(fid,'%3u %3u %10.2e\n',j,i,area(i,j)*sum((TV(h,2)*TVfrac+TV(h,1)*(1-TVfrac))*scale));
            end
        end
        fprintf(fid,'\n');
    end
